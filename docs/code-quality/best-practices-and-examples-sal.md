...
## <a name="opt"></a>_opt\_  
 Çağıran bir null işaretçi geçmesine izin verilmiyor kullanırsanız `_In_` veya `_Out_` yerine `_In_opt_` veya `_Out_opt_`. Bu parametrelerini denetler ve olmamalıdır zaman NULL ise, bir hata döndürür bile bir işlev için geçerlidir. Bir işlev parametresi için beklenmeyen NULL denetleyin ve düzgün bir şekilde dönmek olması iyi bir savunma kodlama uygulama olsa da, bu parametre ek açıklama isteğe bağlı bir türde olabilir gelmez (_*Xxx*_opt\_).  
...
## <a name="wrong-context-in-when"></a>_Tamam yanlış bağlamda\_  
