...
-   Tasarımcıya sürüklediğiniz varlık (veya tasarımcıya sürüklediğiniz özelliği içeren varlık) için bir sorgu döndüren yeni bir yöntem ekler. Yeni yöntemi adına sahip `Get<EntityName>Query`, burada `\<EntityName>` varlık adıdır.

-   Oluşturur bir <xref:System.Windows.FrameworkElement.Loaded> için olay işleyicisini [!INCLUDE[TLA2#tla_ui](../data-tools/includes/tla2sharptla_ui_md.md)] denetimi içeren öğe. Olay işleyicisi çağrılarını `Get<EntityName>Query` varlık verilerle doldurmak için yöntemi alır <xref:System.Windows.Data.CollectionViewSource> kapsayıcının kaynakları ve yapar sonra geçerli öğenin ilk veri öğesi. Varsa bir <xref:System.Windows.FrameworkElement.Loaded> olay işleyicisi zaten mevcut, Visual Studio mevcut olay işleyicisine bu kodu ekler.
...
> [!NOTE]
>  Özel sınıflar gerekir ortak ve, varsayılan olarak, parametresiz bir oluşturucusu olmalıdır. Bunlar, kendi sözdiziminde "nokta" olan can'tbe iç içe geçmiş sınıf. Daha fazla bilgi için bkz: [XAML ve WPF için özel sınıflar](/dotnet/framework/wpf/advanced/xaml-and-custom-classes-for-wpf).
