...
Bir TableAdapter güncelleştirme işlevselliğini ne kadar bilgi ana sorgusunda kullanılabilir olduğuna bağlıdır **TableAdapter Sihirbazı**. Örneğin, birden çok tablodan değerleri getirmek için yapılandırılmış TableAdapters (kullanarak bir `JOIN`), skaler değerler, görünüm veya toplama işlevleri sonuçlarını başlangıçta oluşturulmaz temel veritabanına güncellemelerin olanağı. Ancak, yapılandırabileceğiniz `INSERT`, `UPDATE`, ve `DELETE` el ile komutları **özellikleri** penceresi.
...
|Üye|Açıklama|
|------------|-----------------|
|`TableAdapter.Fill`|TableAdapter'ın sonuçlarını TableAdapter'ın ilişkili veri tablosu doldurur `SELECT` komutu.|
...
TableAdapter bağdaştırıcıları veritabanından okuma ve yazma yapmak için veri komutlarını kullanır. TableAdapter'ın ilk kullanmak `Fill` ilişkili veriler tablonun şeması oluşturmak için temel olarak (ana) sorgu yanı sıra `InsertCommand`, `UpdateCommand`, ve `DeleteCommand` ilişkili komutları `TableAdapter.Update` yöntemi. TableAdapter's çağırma `Update` yöntemi TableAdapter ilk olarak yapılandırıldığında, oluşturulan ifadeleri çalıştırır, ek bir sorgular, eklendi **TableAdapter sorgu Yapılandırma Sihirbazı'nı**.

Bir TableAdapter kullandığınızda, etkili bir şekilde genellikle gerçekleştireceği komutları ile aynı işlemleri gerçekleştirir. Örneğin, bağdaştırıcının çağırdığınızda `Fill` yöntemi, bağdaştırıcı veri komutu çalıştırır kendi `SelectCommand` özelliği ve bir veri okuyucu kullanır (örneğin, <xref:System.Data.SqlClient.SqlDataReader>) sonuç veri tabloya kümesini yüklemek için. Benzer şekilde, bağdaştırıcının çağırdığınızda `Update` yöntemi, uygun komutu çalıştırır (içinde `UpdateCommand`, `InsertCommand`, ve `DeleteCommand` özellikleri) her biri için veri tablosu kaydında değiştirildi.

> [!NOTE]
> Yeterli bilgi ana sorgu ise `InsertCommand`, `UpdateCommand`, ve `DeleteCommand` komutları varsayılan olarak oluşturulan zaman TableAdapter oluşturulur. TableAdapter ana sorgu tek bir tabloyu birden ise `SELECT` deyimi, onu Tasarımcı oluşturmak mümkün olmayacak olası `InsertCommand`, `UpdateCommand`, ve `DeleteCommand`. Bu komutlar oluşturulan değil, çalıştırırken bir hata alabilirsiniz `TableAdapter.Update` yöntemi.

## <a name="tableadapter-generatedbdirectmethods"></a>TableAdapter GenerateDBDirectMethods Özelliği

Ek olarak `InsertCommand`, `UpdateCommand`, ve `DeleteCommand`, TableAdapters doğrudan veritabanına karşı çalışan yöntemleriyle oluşturulur. Bu yöntemleri çağırabilir (`TableAdapter.Insert`, `TableAdapter.Update`, ve `TableAdapter.Delete`) doğrudan veritabanındaki verileri işlemek için. Bu kodunuzdan çağırmak yerine bu tek tek yöntemleri çağırabilir anlamına gelir `TableAdapter.Update` ekler, güncelleştirmeleri ve bekleyen silme işlemek için ilişkili veri tablosu için.
...
|*tableName* `TableAdapter` özelliği|Bir TableAdapter temsil eder. Oluşturulan TableAdapterManager her biri için bir özellik içeriyor `TableAdapter` yönettiği. Örneğin, müşteriler ve siparişler bir tablo içeren bir veri kümesi içeren bir TableAdapterManager ile oluşturur `CustomersTableAdapter` ve `OrdersTableAdapter` özellikleri.|
