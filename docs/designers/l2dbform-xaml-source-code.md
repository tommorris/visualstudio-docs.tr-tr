...
## <a name="window-resource-section"></a>Pencere kaynak bölümü  
 ...
 `<ObjectDataProvider>` Satırları 11 25 aracılığıyla yayılan, etiket bildiren bir <xref:System.Windows.Data.ObjectDataProvider>, adlandırılmış `LoadedBooks`, kullanan bir <xref:System.Xml.Linq.XElement> kaynağı olarak. Bu <xref:System.Xml.Linq.XElement> katıştırılmış bir XML belgesi ayrıştırarak başlatılır (bir `CDATA` öğesi). Katıştırılmış XML belgesi de zaman ayrıştırılır bildirme ve bu boşluk korunur dikkat edin. Bunun nedeni yapılmadı <xref:System.Windows.Controls.TextBlock> ham XML görüntülemek için kullanılan denetimi sahip hiçbir özel XML özellikleri biçimlendirme.  
...
```  
 ...
 Bu seçenek (satırında, 46) mümkün kılar için <xref:System.Windows.Controls.TextBlock> adlı `tbRawXml` bu veri sağlayıcısının bağlayarak ham XML görüntülenecek `Xml` özelliği:  
...
```  
 ...
 Böylece kitap öğelerini geçerli değerlerini görüntülenecek ve iki metin kutusuna bu bölmedeki güncelleştirildi sonra iki yönlü veri bağlama kullanır. Veri bağlama Dinamik özellikler için kullanılan benzer `BookTemplate` veri şablonu:  
  
```  
Text="{Binding Path=Attribute[id].Value}"...Text="{Binding Path=Value}"  
```  
  
 Son kullanıcı Arabirimi bölümde **yeni rehberi Ekle**, veri kullanmaz kendi XAML kodunda; bağlama bunun yerine, bu kod, olay L2DBForm.xaml.cs dosyasındaki kodu işleme bulunabilir.  
