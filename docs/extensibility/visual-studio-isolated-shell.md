...
Visual Studio yalıtılmış Kabuk yan yana çalıştırabileceğiniz bağımsız uygulamalar oluşturmanıza olanak tanıyan diğer Visual Studio sürümleri ile. Visual Studio hizmetleri kullanan ancak özelleştirilmiş bir görünümünü de özel araçlar öncelikle barındırmak için kullanılan ve marka. Visual Studio özellikleri ve menü komut gruplarını, kolayca açma ve kapatma açılabilir. Uygulama başlıkları, uygulama simgeleri ve başlangıç ekranında tamamen özelleştirilebilir. Özelleştirilebilir özelliklerin listesi için bkz: [yalıtılmış Kabuk özelleştirme](../extensibility/customizing-the-isolated-shell.md).  
 ...
 Yalıtılmış Kabuk projesi ile çalışmak için Visual Studio SDK'yı yüklemeniz gerekir. Visual Studio 2015'ten başlayarak, Visual Studio SDK'sını İndirme Merkezi'nden yüklemeyin. Visual Studio kurulumunda bir isteğe bağlı bir özellik olarak eklenmiştir. VS SDK'yı daha sonra da yükleyebilirsiniz. Daha fazla bilgi için bkz: [Visual Studio SDK'sını yükleme](../extensibility/installing-the-visual-studio-sdk.md).  
...
> [!NOTE]
>  Yalıtılmış kabuk tabanlı uygulama dağıtma hakkında daha fazla bilgi için bkz: [izlenecek yol: temel bir yalıtılmış Kabuk uygulama oluşturma](../extensibility/walkthrough-creating-a-basic-isolated-shell-application.md).  
...
-   Yalnızca bunları herhangi diğer Visual Studio Uzantısı'nda kullanırsınız gibi yalıtılmış Kabuk uygulama genişletmek için VSPackages ve Yönetilen Genişletilebilirlik Çerçevesi (MEF) bileşeni bölümleri kullanabilirsiniz. Daha fazla bilgi için bkz: [yalıtılmış Kabuk genişletme](../extensibility/extending-the-isolated-shell.md).  
...
-   Uygulama başlatıldığında Kabuk bazı yönlerini de belirtilebilir. Bunu yapmak için başlangıç giriş noktasını appenvstub.dll çağrısında parametreleri güncelleştirin.  
 ...
 Özelleştirebileceğiniz farklı öğeler hakkında daha fazla bilgi için bkz: [öğeleri yalıtılmış Kabuk](../extensibility/elements-of-the-isolated-shell.md).  
