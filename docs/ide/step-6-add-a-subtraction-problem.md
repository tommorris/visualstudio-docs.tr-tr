...
IntelliSense penceresi araç ipucu  
     ...
     Araç İpucu gösterir **(+ 2 overload(s))**, çağırabilirsiniz anlamına `Next()` iki yolla yöntemi. Böylece birbirinden biraz farklı çalıştığını aşırı farklı sayılar veya bağımsız değişken türleri içerir. Örneğin, kendi aşırı birini bir tamsayı ve bir dize alabilir ancak bir yöntem tek tamsayı bağımsız değişkeni alabilir. Ne yapmak istiyorsunuz üzerinde göre doğru aşırı seçin. Koda eklediğinizde `StartTheQuiz()` yöntemi, daha fazla bilgi IntelliSense penceresinde görünür girdiğiniz hemen `randomizer.Next(`. Aşağıdaki çizimde gösterildiği gibi aşırı yüklemeleri arasında geçiş yapmak için yukarı ve aşağı ok tuşları seçin.  
  
     ![Aşırı İleri &#40; &#41; IntelliSense yönteminde](../ide/media/express_nextoverload.png "Express_NextOverload")  
IntelliSense Next() yöntemi için aşırı yükleme  
