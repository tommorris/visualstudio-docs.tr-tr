...
ms.author: kraigb
manager: ghogen
ms.openlocfilehash: e7ea52c913ec9df94a9e7ad94f8a848acf23f800
...
| Sistem genelinde PYTHONPATH değişkenleri yoksay | Açık | Visual Studio ortamları ve projeleri arama yolları belirtmek için daha doğrudan bir yol sağladığından PYTHONPATH varsayılan olarak sayılır. Bkz: [Python ortamları - arama yolları](python-environments.md#search-paths) Ayrıntılar için. |
| Bağlantılı dosyaları eklerken güncelleştirme arama yolları | Açık | Ayarlanırsa, ekleme, bir [bağlı dosya](python-projects.md#linked-files) projeye güncelleştirmeleri [arama yolları](python-environments.md#search-paths) böylece IntelliSense tamamlanma veritabanında bulunan bağlantılı dosyanın klasörünün içeriğini içerebilir. Bu tür içeriği tamamlama veritabanından dışlamak için bu seçeneği temizleyin. |
...
| Seçenek | Varsayılan | Açıklama |
| --- | --- | --- |
| Komut dosyaları | yok | Tüm ortamlar için etkileşimli windows uygulamak başlatma komut dosyaları için genel bir klasörü belirtir. Bkz: [başlatma komut dosyaları](python-environments.md#startup-scripts). Ancak, bu özellik şu anda çalışmıyor unutmayın. |
