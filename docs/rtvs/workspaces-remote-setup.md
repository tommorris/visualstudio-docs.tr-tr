...
- [Ubuntu üzerinde R hizmetlerini yükleme](#install-r-services-on-ubuntu)
...
### <a name="obtaining-a-self-signed-certificate"></a>Kendinden imzalı bir sertifika edinme
...
3. Çalıştırma `make-ssl-cert` varsayılan otomatik olarak imzalanan SSL sertifikasını oluşturmak için:
    ...
    ```
4. PEM dosyaları ve oluşturulan anahtarı için PFX dönüştürün. Oluşturulan PFX giriş klasörünüzde olmalıdır:
    ```sh
...
## <a name="install-r-services-on-ubuntu"></a>Ubuntu üzerinde R hizmetlerini yükleme
...
1. İndirin, ayıklayın ve yükleme komut dosyası çalıştırma [RTVS arka plan programı paket](https://aka.ms/r-remote-services-linux-binary-current). Gerekli paketler, bağımlılıklarını ve RTVS arka plan programı yüklemeniz gerekir:

    - İndirin:`wget -O rtvs-daemon.tar.gz https://aka.ms/rtvs-daemon-current`
    - Ayıklayın:`tar -xvzf rtvs-daemon.tar.gz`
    - Çalışma yükleyicisi: `sudo ./rtvs-install` . DotNet paket yükleme bize yeni bir güvenilir imzalama anahtarı eklemek gerektirir. Sessiz yükleme ya da Otomasyon için bu komutu kullanmak için `sudo ./rtvs-install -s`.
    

1. Etkinleştirin ve arka plan programı başlatın:

    - Etkinleştir:`sudo systemctl enable rtvsd`
    - Arka plan programı'nı başlatın:`sudo systemctl start rtvsd`

1. Arka plan programı, bu komutu çalıştırmak çalışıp çalışmadığını denetleyin `ps -A -f | grep rtvsd`. Olarak çalışan bir işlemi görmelisiniz `rtvssvc` kullanıcı. Artık bu linux makineye url suing visual Studio için R araçlarından bağlanmak mümkün olması gerekir.

Yapılandırmak için `rtvs-daemon`, bkz: `man rtvsd`.
...
Yukarıdaki değişiklikleri yaptıktan sonra bilgisayarı yeniden başlatın. Ardından olduğundan emin olun `RHostBrokerService` ve `RUserPofileService` ya da Görev Yöneticisi (Hizmetleri sekmesi) üzerinden çalışan veya `services.msc`.
