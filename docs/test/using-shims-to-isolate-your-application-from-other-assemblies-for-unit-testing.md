...
-   Visual Studio Enterprise  
 ...
 Bkz: [Video (1-h 16): Visual Studio 2012'de beklemediğiniz sınanabilir kodu Fakes ile test etme](http://go.microsoft.com/fwlink/?LinkId=261837)  
  
## <a name="in-this-topic"></a>Bu konuda  
 Bu konuda bilgi edineceksiniz aşağıda verilmiştir:  
...
-   [Dolgular ile testleri yazma](#WriteTests)  
 ...
 [Dolgular için çeşitli yöntemler](../test/using-shims-to-isolate-your-application-from-other-assemblies-for-unit-testing.md#BKMK_Shim_basics)  
...
-   [Örnek yöntemleri (için tüm örnekleri)](../test/using-shims-to-isolate-your-application-from-other-assemblies-for-unit-testing.md#BKMK_Instance_methods__for_all_instances_)  
  
-   [Örnek yöntemleri (örneğin bir çalışma zamanı)](../test/using-shims-to-isolate-your-application-from-other-assemblies-for-unit-testing.md#BKMK_Instance_methods__for_one_instance_)  
...
-   [Bağlama arabirimleri](../test/using-shims-to-isolate-your-application-from-other-assemblies-for-unit-testing.md#BKMK_Binding_interfaces)  
 ...
 [Varsayılan davranışı değiştirme](../test/using-shims-to-isolate-your-application-from-other-assemblies-for-unit-testing.md#BKMK_Changing_the_default_behavior)  
  
 [Algılama ortamı erişir](../test/using-shims-to-isolate-your-application-from-other-assemblies-for-unit-testing.md#BKMK_Detecting_environment_accesses)  
...
```  
 ...
 Bu yöntem sınama özellikle sorunlu programa bağımlı olduğundan dolayı `DateTime.Now`, bilgisayara bağlı bir yöntem kullanıcının saat, bir ortam bağımlı, belirleyici olmayan yöntemi. Ayrıca, `DateTime.Now` bir saplama türü burada kullanılamaz statik bir özelliktir. Bu sorunu birim testi yalıtım sorunun belirtisi: kendi mantığı ortamda bağımlı olduğundan dolayı doğrudan çağırmak API'leri veritabanına, web Hizmetleri ile iletişim kurmak ve benzeri programlar birim testi için sabit.  
  
 Dolgu türleri kullanıldığı budur. Dolgu türleri herhangi bir .NET yöntemi, bir kullanıcı tanımlı temsilci saptıran için bir mekanizma sağlar. Kod-Fakes oluşturma tarafından oluşturulan dolgusu türleri ve dolgusu türleri diyoruz, temsilciler yeni yöntem uygulamalarını belirtmek için kullandıkları.  
...
```  
 ...
 Her dolgusu bağlamı düzgün bir şekilde silmek için önemlidir. Altın kural, her zaman çağrısı `ShimsContext.Create` içine bir `using` kayıtlı dolgular, uygun temizleme emin olmak için deyimi. Örneğin, bir dolgu yerini alan bir test yöntemi için kayıt `DateTime.Now` yöntemi her zaman döndüren bir temsilci ile ilk Ocak, 2000. Test yöntemi kayıtlı dolgusunu temizlemek unutursanız, test çalışması kalan her zaman döndürecektir DateTime.Now olarak ilk, Ocak 2000 değeri. Bu suprising ve kafa karıştırıcı olabilir.  
...
```  
 ...
 Her Dolgu türü iki oluşturucular sunan unutmayın. Varsayılan Oluşturucu, yeni bir örneği, yalnızca Oluşturucusu dolgular bağımsız değişkeni kullanılmalıdır gibi bir shimmed örneği alan oluşturucu çalışırken gerekli olduğunda kullanılmalıdır:  
...
```  
 ...
 ShimMyClass oluşturulan tür yapısını followoing kod benzer:  
...
###  <a name="BKMK_Private_methods"></a>Özel yöntemler  
 Fakes Kod Oluşturucu yalnızca imza, yani parametre türleri ve dönüş türü görünür görünür türlerine sahip özel yöntemler dolgusu özelliklerini oluşturur.  
...
##  <a name="BKMK_Changing_the_default_behavior"></a>Varsayılan davranışı değiştirme  
 ...
 Davranış açıkça ayarlı değil, statik tarafından döndürülen örnek kullanacak `ShimsBehaviors.Current` özelliği. Varsayılan olarak, bu özellik oluşturur bir davranış döndürür bir `NotImplementedException` özel durum.  
...
## <a name="see-also"></a>Ayrıca Bkz.  
 ...
 [Video (1-h 16): Visual Studio 2012'de beklemediğiniz sınanabilir kodu Fakes ile test etme](http://go.microsoft.com/fwlink/?LinkId=261837)
