---
title: VSTO eklentileri diğer Office Çözümlerinden kodu çağırma
ms.custom: ''
ms.date: 02/02/2017
ms.technology:
- office-development
...
## <a name="types-of-solutions-that-can-call-code-in-an-add-in"></a>Bir eklenti kod çağırabilir çözüm türleri  
...
### <a name="define-the-class-you-want-to-expose-to-other-solutions"></a>Diğer çözümleri kullanıma sunmak istediğiniz sınıf tanımlama  
...
4.  Uygulama <xref:System.Runtime.InteropServices.ClassInterfaceAttribute> özniteliği sınıfınıza ve bu öznitelik ayarlanırsa **hiçbiri** değerini <xref:System.Runtime.InteropServices.ClassInterfaceType> numaralandırması.  
  
5.  Bu sınıf işlem dışı istemcilere kullanıma sunmak istiyorsanız, aşağıdakileri yapmanız gerekebilir:  
  
    -   Sınıfından türetilen <xref:System.Runtime.InteropServices.StandardOleMarshalObject>. Daha fazla bilgi için bkz: [kullanıma işlem dışı istemcilere sınıfları](#outofproc).  
  
    -   Ayarlama **kaydetmek için COM birlikte çalışma** arabirimi tanımladığınız yerlerde projesinde özelliği. Bu özellik yalnızca istemcilerin VSTO eklenti çağırmak için erken bağlama kullanmasını etkinleştirmek istiyorsanız gereklidir.  
  
 Aşağıdaki kod örneğinde gösteren bir `AddInUtilities` ile sınıf bir `ImportData` diğer çözümleri tarafından çağrılan yöntemi. Bu kodu daha geniş bir anlatım bağlamında görmek için bkz: [izlenecek yol: çağrı kod bir VSTO eklentide VBA'dan](../vsto/walkthrough-calling-code-in-a-vsto-add-in-from-vba.md).  
  
 [!code-csharp[Trin_AddInInteropWalkthrough #3](../vsto/codesnippet/CSharp/Trin_AddInInteropWalkthrough/AddInUtilities.cs#3)]
 [!code-vb[Trin_AddInInteropWalkthrough#3](../vsto/codesnippet/VisualBasic/Trin_AddInInteropWalkthrough/AddInUtilities.vb#3)]  
  
### <a name="expose-classes-to-vba"></a>Sınıfları VBA kullanımına sunma  
...
### <a name="override-the-requestcomaddinautomationservice-method"></a>RequestComAddInAutomationService yöntemini geçersiz kılın  
 Aşağıdaki kod örneğinde nasıl geçersiz kılınacağı gösterilmektedir <xref:Microsoft.Office.Tools.AddInBase.RequestComAddInAutomationService%2A> içinde `ThisAddIn` VSTO eklentinizi sınıfta. Adlı bir sınıf tanımladığınız örnek varsayar `AddInUtilities` diğer çözümlerde oluşturmak istediğiniz. Bu kodu daha geniş bir anlatım bağlamında görmek için bkz: [izlenecek yol: çağrı kod bir VSTO eklentide VBA'dan](../vsto/walkthrough-calling-code-in-a-vsto-add-in-from-vba.md).  
...
1.  Alma <xref:Microsoft.Office.Core.COMAddIn> gösterilen için VSTO eklentisi temsil eden nesne. İstemciler tüm kullanılabilir VSTO eklentileri kullanarak erişim `Application.COMAddIns` Office uygulama konağının nesne modelinde özelliği.  
...
### <a name="access-objects-from-vba-solutions"></a>VBA çözümlerinden gelen nesnelere erişme  
 Aşağıdaki kod örneğinde VBA VSTO eklenti tarafından sunulan bir yöntemi çağırmak için nasıl kullanılacağı gösterilmektedir. Bu VBA makrosu adlı bir yöntemi çağırır `ImportData` bir VSTO adlı eklenti tanımlanan **ExcelImportData**. Bu kodu daha geniş bir anlatım bağlamında görmek için bkz: [izlenecek yol: çağrı kod bir VSTO eklentide VBA'dan](../vsto/walkthrough-calling-code-in-a-vsto-add-in-from-vba.md).  
...
### <a name="access-objects-from-non-vba-solutions"></a>VBA olmayan çözümlere erişimi nesneleri  
